 ...
 * @return the method return variable, a heap-allocated pointer, should be free
 * from caller;
 *
 */
                       ...
                       class_file_t *clazz)
{
    ...
    while (pc < code.code_length) {
        ...
        case i_invokestatic: {
            ...
            /* if not found, add specify class path. this can be remove by
             * record path infomation in class heap */
            if (!target_class) {
            ...
            if (!target_class) {
                /* 7 = ".class" + 1 */
                char *tmp = malloc((strlen(class_name) + 7 + strlen(prefix)) *
                                   sizeof(char));
                strcpy(tmp, prefix);
                strcat(tmp, class_name);
            ...
            if (own_method->access_flag & ACC_NATIVE) {
                ...
                if (method_descriptor[strlen(method_descriptor) - 1] == 'V') {
                    ...
                    /* method return long */
                } else if (method_descriptor[strlen(method_descriptor) - 1] ==
                           'J') {
        ...
        case i_ldc: {
            ...
            default:
                fprintf(stderr,
                        "ldc only support int and string, but get tag %d\n",
                        info->tag);
                exit(1);
            }
            pc += 2;
        ...
        case i_tableswitch: {
            ...
            uint8_t defaultbyte1 = code_buf[pc],
                    defaultbyte2 = code_buf[pc + 1],
                    defaultbyte3 = code_buf[pc + 2],
                    defaultbyte4 = code_buf[pc + 3];
            int32_t _default = ((defaultbyte1 << 24) | (defaultbyte2 << 16) |
                                (defaultbyte3 << 8) | defaultbyte4);
            pc += 4;
            uint8_t lowbyte1 = code_buf[pc], lowbyte2 = code_buf[pc + 1],
                    lowbyte3 = code_buf[pc + 2], lowbyte4 = code_buf[pc + 3];
            int32_t low = ((lowbyte1 << 24) | (lowbyte2 << 16) |
                           (lowbyte3 << 8) | lowbyte4);
            pc += 4;
            uint8_t highbyte1 = code_buf[pc + 1], highbyte2 = code_buf[pc + 1],
                    highbyte3 = code_buf[pc + 2], highbyte4 = code_buf[pc + 3];
            int32_t high = ((highbyte1 << 24) | (highbyte2 << 16) |
            ...
            if (key >= low && key <= high) {
                ...
                uint8_t byte1 = code_buf[index], byte2 = code_buf[index + 1],
                        byte3 = code_buf[index + 2],
                        byte4 = code_buf[index + 3];
                uint32_t addr =
        ...
        case i_getstatic: {
            ...
            if (!new_clazz) {
                ...
                *new_clazz = get_class(class_file);
                ...
                if (method) {
                    ...
                    stack_entry_t *exec_res =
                        execute(method, own_locals, new_clazz);
                    assert(exec_res->type == STACK_ENTRY_NONE &&
                           "<clinit> must be no return");
                    free(exec_res);
                }
                free(tmp);
            }
            ...
            field_t *field =
                find_field(field_name, field_descriptor, new_clazz);

            /* find super class */
            while (!field) {
                char *super_name =
                    find_class_name_from_index(clazz->super_class, clazz);
            ...
            case 'B':
                ...
                break;
            case 'C':
                /* Unicode character code point in the Basic Multilingual Plane,
                 * encoded with UTF-16 */
                push_byte(op_stack, field->value->value.char_value);
                break;
            case 'D':
        ...
        case i_getfield: {
            ...
            while (!addr) {
                char *super_name = find_class_name_from_index(
                    obj->type->super_class, obj->type);
                assert(super_name && "cannot find field");
            ...
            if (!new_class) {
                ...
                *new_class = get_class(class_file);
                ...
                if (method) {
                    local_variable_t own_locals[method->code.max_locals];
                    stack_entry_t *exec_res =
                        execute(method, own_locals, new_class);
                    assert(exec_res->type == STACK_ENTRY_NONE &&
                           "<clinit> must be no return");
                    free(exec_res);
                }
        ...
        case i_dup2: {
            stack_entry_t element = top(op_stack);
            /* category 2 */
            if (element.type == STACK_ENTRY_LONG ||
                element.type == STACK_ENTRY_DOUBLE) {
                op_stack->store[op_stack->size] =
                    op_stack->store[op_stack->size - 1];
                op_stack->size++;
                pc += 1;
            ...
            /* category 1 */
                ...
                op_stack->store[op_stack->size] =
                    op_stack->store[op_stack->size - 2];
                op_stack->store[op_stack->size + 1] =
                    op_stack->store[op_stack->size - 1];
                op_stack->size += 2;
                pc += 1;
        ...
        case i_invokedynamic: {
            ...
            char *arg = NULL;
            /* we only support makeConcatWithConstants (string concatenation),
             * this mean argument must be only one string */
            /* FIXME: if string contain '\0' character, it will cause error */
            assert(bootstrap_method->num_bootstrap_arguments == 1 &&
                   "only support makeConcatWithConstants");
            for (int i = 0; i < bootstrap_method->num_bootstrap_arguments;
                 ++i) {
                arg = get_string_utf(&clazz->constant_pool,
                                     bootstrap_method->bootstrap_arguments[i]);
            }

            /* each \u0001 mean "1" in unicode, this should be replace by actual
             * argument from stack, others are constant characters */
            ...
            if (method->access_flag & ACC_NATIVE) {
                ...
                if (method_descriptor[strlen(method_descriptor) - 1] == 'V') {
                    ...
                    free(exec_res);
                               ...
                               'C' ||
                           method_descriptor[strlen(method_descriptor) - 1] ==
                               'I') {
                    void *exec_res = ptr_native_method(method, own_locals);
                    if (exec_res) {
                        push_int(op_stack, *(int32_t *) exec_res);
        ...
        case i_multianewarray: {
            ...
            char *type =
                (char *) (get_constant(
                              &clazz->constant_pool,
                              get_class_name(&clazz->constant_pool, index)
                                  ->string_index))
                    ->info;
            assert(dimensions == 2 && strcmp(type, "[[I") == 0 &&
                   "this VM only support two dimension integer array");
...
int main(int argc, char *argv[])
{
    ...
    *clazz = get_class(class_file);
    ...
    for (int i = 0; i < class_heap.length; ++i) {
        method_t *method =
            find_method("<clinit>", "()V", class_heap.class_info[i]->clazz);
        if (method) {
            local_variable_t own_locals[method->code.max_locals];
            stack_entry_t *exec_res =
                execute(method, own_locals, class_heap.class_info[i]->clazz);
            assert(exec_res->type == STACK_ENTRY_NONE &&
                   "<clinit> must be no return");
            free(exec_res);
        }
